...
var stepAmount = 1; //čas koraka v milisekundah
let paused = true;
let stepMode = false;
...
const animate = (time) => {
    ...
    if (stepMode == false) {
        ...
        if (paused == true) {
            ...
            //recordedTime = Math.ceil(time - timeSinceTimePaused);
    ...
    document.getElementById("coords").innerHTML = "x: " + (cube.position.x).toFixed(6) + ", y: " + (cube.position.y).toFixed(2) + ", z: " + (cube.position.z).toFixed(2);
    document.getElementById("time").innerHTML = "Time (recorded): " + (recordedTime).toFixed(2) + "ms" + ", Time (paused): " + (timePaused).toFixed(2) + "ms";
...
//Začetne koordinate
const move1 = new TWEEN.Tween({ x: 0, y: 0, z: 0 })
...
const move2 = new TWEEN.Tween({ x: 1, y: 0, z: 0 })
    .to({ x: 0, y: 0, z: 0 }, 1000)
    .onUpdate((coords) => {
        cube.position.x = coords.x;
        cube.position.y = coords.y;
